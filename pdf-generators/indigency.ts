import PDFDocument, { lineGap, page } from "pdfkit";
import path from "path";
import calculateAge from "../Helpers/calculateAge";
import transformDateToText from "../Helpers/transformText";
...
function generateIndigency(request: {
    ...
    doc.opacity(0.02)
    doc.image(path.join(__dirname, "../../public/brgy-sandiat-centro-logo.jpg"), 40, 170, {width: (doc.page.width - 80), height: (doc.page.width - 80)});
    
    doc.opacity(0.15)
    doc.image(path.join(__dirname, "../../public/assets/images/phflag.jpg"), 0, 0, {width: 500})
    doc.opacity(1)
    ...
    doc.image(path.join(__dirname, "../../public/san-manuel-logo.png"), 70, 40, {width: 70, height: 70})
    doc.image(path.join(__dirname, "../../public/brgy-sandiat-centro-logo.jpg"), (doc.page.width - 70) - 70, 40, {width: 70, height: 70})
    ...
    .moveDown(1)
    .font('Helvetica')
    .text(`This is to certify that ${request.name.toUpperCase()}, ${calculateAge(new Date(request.dateOfBirth))} years of age, ${request.maritalStatus}, Filipino and a Bonafede resident of this Barangay Sandiat Centro, San Manuel, Isabela.`, {indent: 72})
    .moveDown(1)
    ...
    .moveDown(1)
    .text(`Given this ${transformDateToText(new Date())} at Sandiat Centro, San Manuel, Isabela.`, {indent: 72})
    .moveDown(2)
    .font('Helvetica-Bold')
